...
class DeciduousTrees {
    ...
    randomTreeData(startingStructure, startingDepth) {
        ...
        const maxDepth = this.r.randomInt(this.options.MAX_DEPTH * 0.7, this.options.MAX_DEPTH);
        //Math.round(this.options.MAX_DEPTH * this.r.random(0.3,0.6));
        if (depth < maxDepth) {
            const branchChance = depth == this.options.MAX_DEPTH - 1
    ...
    buildBranch(baseLength, distanceFromTip, distanceFromRoot, fullTreeDepth, minRad, maxRad) {
        ...
        const cylGeom = new THREE.CylinderGeometry(radiusTop, radiusBottom, referenceLength, 8);
        ...
        const branchCol = this.c.mixHexCols(this.options.COLOR_BTM, this.options.COLOR_TOP, propBtm, propTop);
        ...
        if (distanceFromTip < 2) {
            this.makeLeavesAround(branch.tip, this.r.randomInt(this.options.LEAF_DENSITY * 2, this.options.LEAF_DENSITY * 3), this.options.LEAF_COLS, 2 * this.options.LEAF_SIZE, 0, 0, 2 * this.options.LEAF_SIZE);
        }
        return branch;
    }
    /**
    ...
    makeLeavesAround(obj3d, numLeaves, colors, leafRadius, yAdjust, rAdjust, leafWidth) {
        for (let i = 0; i < numLeaves; i++) {
            ...
            newLeaf.position.x += this.r.randomSign(this.r.random(0.75) * circleSize + rAdjust);
            newLeaf.position.z += this.r.randomSign(this.r.random(0.75) * circleSize + rAdjust);
     ...
     */
    buildTree(treeData, branchLength, depth, height, fullTreeDepth, maxBranchRad) {
        let fanRads = this.de2ra(this.options.ANGLE_MIN +
            this.r.random(this.options.ANGLE_MAX - this.options.ANGLE_MIN));
        // Don't start fanning out too low in the tree.
        if (height < 2) {
            //fanRads = fanRads / 2;
        }
        const root = this.buildBranch(branchLength, depth, height, fullTreeDepth, this.options.BRANCH_R_MIN, maxBranchRad);
        ...
        if (this.options.MUSHROOMS && height < bottomThreshold) {
            console.log("ADDING MUSHROOMS AT HEIGHT " + height + " /" + fullTreeDepth);
            for (let i = 0; i < numClusters; i++) {
                this.addMushroomsTo(root, maxBranchRad, branchLength);
            }
        }
        for (let i = 0; i < treeData.length; i++) {
            const newBranch = this.buildTree(treeData[i], branchLength * this.options.LENGTH_MULT, this.depthOfArray(treeData[i]), height + 1, fullTreeDepth, maxBranchRad);
    ...
    treeWithRoots(treeData, branchLength, depth, height, fullTreeDepth, maxBranchRad) {
        if (this.rainbow) {
            ...
            this.options.COLOR_BTM = this.c.brightenByMult(this.options.COLOR_TOP, 0.3);
        ...
        const body = this.buildTree(treeData, branchLength, depth, height, fullTreeDepth, maxBranchRad);
        ...
        for (let i = 0; i < numRoots; i++) {
            ...
            const rootLength = this.r.random(branchLength * 0.05, branchLength * 0.25);
    ...
    getTree(options) {
        ...
        return this.treeWithRoots(data, this.options.BRANCH_L, this.depthOfArray(data), 0, this.depthOfArray(data), this.options.BRANCH_R_MAX);
    }
    ...
    initOptions(opts) {
        ...
        for (let i = 0; i < numLeafColors; i++) {
            leafColors.push(this.c.variationsOn(this.LEAF_BASE_COLOR, colorVariability));
        }
        const btm_color = Math.random() < 0.4
            ? top_color
            : this.c.mixHexCols(this.c.randomHex(), this.c.brightenByMult(top_color, 0.4), 0.6, 0.4);
        const options = {
            RAINBOW: false,
            COLOR_INTENSE: false,
...
exports.default = DeciduousTrees;
