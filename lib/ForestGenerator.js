...
const DeciduousTrees_1 = require("./DeciduousTrees");
const ConiferousTrees_1 = require("./ConiferousTrees");
// import Moon from './Moon'
class ForestGenerator {
    constructor(forestOptions, treeOptions) {
        this.forestOptions = forestOptions;
        this.treeOptions = treeOptions;
        this.c = new Colors_1.default();
        this.r = new Randoms_1.default();
        this.palette = [];
        this.GROUNDLEAF_WIDTH = this.r.random(0.3, 0.8);
        this.NUM_FRAMES = 100;
        this.trees = [];
        this.rocks = [];
        this.RIDGE_Z1 = 700;
        ...
        this.startingNoise = this.r.randomInt(this.noise.length / 2);
        this.sceneWidth = 600;
        ...
        this.camera = new THREE.PerspectiveCamera(50, this.sceneWidth / this.sceneHeight, 1, 1500);
        this.aLittleHigherPos = this.scene.position;
        ...
        this.decid = new DeciduousTrees_1.default(this.treeOptions);
        this.conif = new ConiferousTrees_1.default(this.treeOptions);
        this.initColors();
    ...
    initColors() {
        ...
        this.SKY_COL = this.NIGHT_MODE
            ? this.c.variationsOn(this.treeOptions.RAINBOW ? "#222222" : "#4d6876", this.treeOptions.RAINBOW ? 50 : 120)
            : this.c.variationsOn("#bdeff1", 150);
        this.GROUND_COL = this.NIGHT_MODE
            ? this.c.variationsOn(this.treeOptions.RAINBOW ? "#111111" : "#40523C", this.treeOptions.RAINBOW ? 40 : 80)
        ...
        const stoneGrey = this.c.greyHex(this.NIGHT_MODE ? this.r.randomInt(20, 60) : this.r.randomInt(90, 130));
    ...
    makeGIF() {
        ...
        const gifBuffer = new Buffer(this.sceneWidth * this.sceneHeight * this.NUM_FRAMES);
        const gif = new omggif.GifWriter(gifBuffer, this.sceneWidth, this.sceneHeight, {
        ...
        fs_1.writeFileSync("./images/" + this.filename + ".gif", gifBuffer.slice(0, gif.end()));
    ...
    savePNG(pixelData, width, height) {
        ...
        png.pack().pipe(fs_1.createWriteStream("./images/" + this.filename + ".png"));
    }
    /**
    ...
    convertRGBAto8bit(rgbaBuffer, palette) {
        ...
        let grndColInt = (this.forestOptions.MIRROR) ? skyColInt : this.c.parseHex(this.GROUND_COL);
        let blend0 = this.c.parseHex(this.c.mixHexCols(this.SKY_COL, this.GROUND_COL, 0.9, 0.1));
        let blend1 = this.c.parseHex(this.c.mixHexCols(this.SKY_COL, this.GROUND_COL, 0.7, 0.3));
        let blend2 = this.c.parseHex(this.c.mixHexCols(this.SKY_COL, this.GROUND_COL, 0.5, 0.5));
        let blend3 = (this.forestOptions.MIRROR) ? blend1 : this.c.parseHex(this.c.mixHexCols(this.SKY_COL, this.GROUND_COL, 0.3, 0.7));
        for (let i = 0; i < rgbaBuffer.length; i += 4) {
            let color = (rgbaBuffer[i] << 16) + (rgbaBuffer[i + 1] << 8) + rgbaBuffer[i + 2];
            // stripes
    ...
    hemisphereMesh(col, radius) {
        ...
        const sphGeom = new THREE.SphereGeometry(radius, widthSegments, heightSegments, 0, Math.PI * 2, 0, Math.PI / 2);
    ...
    bushColors() {
        const cols = [];
        let bushBase = this.c.mixHexCols(this.c.randomHex(), this.decid.options.COLOR_BTM, 0.3, 0.7);
    ...
    bush(height, width, colors, leafSize) {
        ...
        for (let i = 0; i < height * 4; i++) {
            ...
            this.makeLeavesAround(clump, this.r.randomInt(20, 40), colors, leafSize, 0, 0, leafSize);
            ...
            clump.position.y = this.r.random(0, height * 0.7 -
    ...
    buildRocks() {
        ...
        const baseGrey = this.c.greyHex(this.NIGHT_MODE ? this.r.randomInt(10, 50) : this.r.randomInt(30, 100));
        const rockCol = (this.forestOptions.MIRROR) ? this.GROUND_COL : this.c.mixHexCols(this.GROUND_COL, baseGrey, 0.6, 0.4);
        ...
        for (let i = 0; i < numRocks; i++) {
            ...
            rock.scale.x = (this.forestOptions.MIRROR) ? this.r.random(1, 13) : this.r.random(0.5, 1);
            rock.scale.z = (this.forestOptions.MIRROR) ? this.r.random(1, 13) : this.r.random(0.5, 1);
    ...
    buildBushes() {
        ...
        const leafSize = this.r.random(this.decid.options.LEAF_SIZE, this.decid.options.LEAF_SIZE * 2);
        const leafWidth = this.r.random(0.7, 1);
        const backBushCols = [];
        if (this.rainbow) {
            backBushCols.push(this.c.brightenByAmt(bushColors[0], this.r.random(-100, -40)));
        }
        else {
            for (let i = 0; i < bushColors.length; i++) {
                backBushCols.push(this.c.brightenByAmt(bushColors[i], this.r.random(-10, 10)));
            }
        }
        console.log("{}{} " + numBushes);
        const numBackBushes = Math.max(60, this.NUM_TREES * 0.6);
        for (let i = 0; i < this.NUM_TREES * 0.6; i++) {
            const newBush = this.bush(this.r.random(bushHeight * 50, bushHeight * 80), bushWidth * 8, backBushCols, leafSize * 4
        ...
        for (let i = 0; i < numBushes; i++) {
            //console.log("bush "+i);
            const newBush = this.bush(this.r.random(bushHeight * 0.5, bushHeight * 1.2), bushWidth, bushColors, leafSize
     ...
     */
    makeLeavesAround(obj3d, numLeaves, colors, leafRadius, yAdjust, rAdjust, leafWidth) {
    ...
    buildClouds() {
        ...
        for (let i = 0; i < numClumps; i++) {
            ...
            for (let j = 0; j < cloudsPerClump; j++) {
                const cloud = this.circleMesh(cloudCol, this.r.random(cloudMaxRad, 0.5));
     ...
     */
    buildForest() {
        const numTrees = this.r.randomInt(this.NUM_TREES * 0.5, this.NUM_TREES * 1.5);
        ...
        for (let i = 0; i < this.NUM_TREES * 2; i++) {
            ...
            console.log("tree " + i);
            const treetype = this.forestOptions.TREE_TYPE == "coniferous" ? this.conif : this.decid;
            ...
            if (i < this.NUM_TREES) {
                ...
                treetype.options.COLOR_BTM = this.c.mixHexCols(treetype.options.COLOR_BTM, this.GROUND_COL, 1 - groundProp, groundProp);
            ...
            wrappedTree.position.x = this.r.randomSign(xPositions[i] / 2 + (i < 20 ? this.r.random(5, 8) : 0));
            if (i < this.NUM_TREES) {
                ...
                this.makeLeavesAround(newTree, 8 + this.r.randomInt(24), this.VEG_COLS, groundLeafSize, 0, this.r.random(treetype.options.BRANCH_R_MAX, treetype.options.BRANCH_R_MAX * 2 * newTree.scale.x), this.GROUNDLEAF_WIDTH);
                this.makeLeavesAround(newTree, 8 + this.r.randomInt(24), this.VEG_COLS, this.pickLeafSize(), 0, this.r.random(treetype.options.BRANCH_R_MAX, treetype.options.BRANCH_R_MAX * 2 * newTree.scale.x), this.GROUNDLEAF_WIDTH);
                //}
            }
            else {
                ...
                wrappedTree.position.z = this.r.random(farEdge * 0.7, this.RIDGE_Z1 * 0.95);
                // let's test grouping these all closer.
                wrappedTree.position.x = this.r.randomSign(xPositions[i] + (i < 12 ? this.r.random(3, 5) : 0));
            }
            wrappedTree.scale.x = wrappedTree.scale.y = wrappedTree.scale.z = this.r.random(0.8, 1.8);
        ...
        if (!this.forestOptions.MIRROR) {
            for (let i = 0; i < this.NUM_TREES * 12; i++) {
                ...
                this.makeLeavesAround(clump, this.r.randomInt(0, 15), this.VEG_COLS, groundLeafSize, 0, 0, this.GROUNDLEAF_WIDTH);
    ...
    grassBlade(grassWidth) {
        ...
        const root = this.grassSegment(0.01, grassHeight, 0, grassHeight, grassWidth, grassWidth, grassCol);
        let workingRoot = root;
        for (let i = 0; i < grassHeight; i++) {
            let segment = this.grassSegment(this.treeOptions.BRANCH_L / 4, grassHeight - i, i, grassHeight, grassWidth * ((grassHeight - (i + 1)) / grassHeight), grassWidth * ((grassHeight - i) / grassHeight), grassCol);
            segment.scale.x = 0.1;
     ...
     */
    grassSegment(baseLength, distanceFromTip, distanceFromRoot, fullTreeDepth, minRad, maxRad, branchCol) {
        ...
        const radiusTop = baseRadius(Math.max(0, distanceFromTip - 1), distanceFromRoot + 1);
        const cylGeom = new THREE.CylinderGeometry(radiusTop, radiusBottom, length, 4);
        ...
        for (let i = 0; i < this.trees.length * 2; i++) {
            ...
            for (let j = 0; j < numInPatch; j += numInCluster) {
                ...
                if (this.forestOptions.MIRROR) {
                    ...
                    clusterPos = {
                        x: rock.position.x + this.r.randomSign(rock + this.exp01(this.r.random(0, 1)) * rock.radius * 2),
                        y: rock.position.y + rock.radius * rock.scale.y,
                        z: rock.position.z + this.r.randomSign(rock + this.exp01(this.r.random(0, 1)) * rock.radius * 2),
                    };
                }
                else {
                    clusterPos = (i > this.trees.length - 1) ?
                        {
                        ...
                        : {
                            x: this.trees[i].position.x + this.r.randomSign(this.treeOptions.BRANCH_R_MAX + this.exp01(this.r.random(0, 1)) * patchSize),
                            y: this.trees[i].position.y,
                            z: this.trees[i].position.z + this.r.randomSign(this.treeOptions.BRANCH_R_MAX + this.exp01(this.r.random(0, 1)) * patchSize)
                        };
                }
                for (let k = 0; k < numInCluster; k++) {
                    let blade = this.r.randomFrom(sourceBlades).clone();
                    blade.position.x =
                        clusterPos.x + this.r.randomSign(this.exp01(this.r.random(0, 1)) * clusterSize);
                    blade.position.y = clusterPos.y;
                    blade.position.z =
                        clusterPos.z + this.r.randomSign(this.exp01(this.r.random(0, 1)) * clusterSize);
    ...
    buildStars() {
        ...
        for (let i = 0; i < numStars; i++) {
            ...
            star.position.y = this.r.random(-700, (this.forestOptions.MIRROR) ? 1400 : 700);
        }
    ...
    generateSceneGIF(numFrames, filename) {
        ...
        console.log(JSON.stringify(this.forestOptions) +
            "\n" +
...
exports.default = ForestGenerator;
