...
class ConiferousTrees {
    constructor(options) {
        ...
        this.m = new Meshes_1.default(this.options);
     ...
     */
    buildBranch(baseLength, distanceFromTip, distanceFromRoot, fullTreeDepth, minRad, maxRad
    // TODO This came from the original ConiferousTrees
    // overridecolor
    ) {
        ...
        const baseRadius = function (distFromTip, distFromRoot) {
            const fromBottom = minRad +
                ((fullTreeDepth - distFromRoot) / fullTreeDepth) * (maxRad - minRad);
            const fromTop = minRad + (distFromTip / fullTreeDepth) * (maxRad - minRad);
            return fromTop;
        };
        ...
        const cylGeom = new three_1.default.CylinderGeometry(radiusTop, radiusBottom, length, 8);
        const sphGeom = new three_1.default.SphereGeometry(radiusTop, 2, 2);
        let hex;
        const propBtm = (fullTreeDepth - distanceFromRoot) / fullTreeDepth;
        const propTop = 1 - propBtm;
        const branchCol = this.rainbow
            ? this.c.randomHex()
            : this.c.mixHexCols(this.options.COLOR_BTM, this.options.COLOR_TOP, propBtm, propTop);
        ...
        const material = new three_1.default.MeshBasicMaterial({
            vertexColors: three_1.default.FaceColors,
            overdraw: 0.5
        });
        const cylinder = new three_1.default.Mesh(cylGeom, material);
        cylinder.position.y = length / 2;
        const sphere = new three_1.default.Mesh(sphGeom, material);
        const tip = new three_1.default.Object3D();
        tip.position.y = length;
        tip.add(sphere);
        const branch = new three_1.default.Object3D();
        branch.add(cylinder);
        branch.add(tip);
        branch.tip = tip;
        ...
        if (distanceFromTip < 2) {
            console.log("LEAVES");
            this.makeLeavesAround(branch.tip, this.r.randomInt(this.options.LEAF_DENSITY * 2, this.options.LEAF_DENSITY * 3), this.options.LEAF_COLS, this.options.LEAF_SIZE, 0, 0, this.options.LEAF_SIZE / 2);
        }
        // TODO This came from the original ConiferousTrees
    ...
    makeLeavesAround(obj3d, numLeaves, colors, leafRadius, yAdjust, rAdjust, leafWidth) {
        ...
        for (let i = 0; i < numLeaves; i++) {
            const sheath = new three_1.default.Object3D();
            ...
            newLeaf.rotation.x = this.r.random(-this.options.ANGLE_MAX, -this.options.ANGLE_MIN);
            sheath.add(newLeaf);
     ...
     */
    buildTree(treeData, branchLength, depth, height, fullTreeDepth, maxBranchRad, corePiece) {
        let baseTwist = this.r.random(Math.PI * 2);
        const mainTrunk = this.depthOfArray(treeData) == fullTreeDepth;
        const minBranchRad = Math.min(this.options.BRANCH_R_MIN, maxBranchRad * 0.5);
        // TODO This came from the original ConiferousTrees
        // let testcolor = depth == 0 ? "#FF0000" : depth == 1 ? "#0000FF" : "#00FF00";
        const root = this.buildBranch(0.1, fullTreeDepth, height, fullTreeDepth, minBranchRad, maxBranchRad
        // TODO This came from the original ConiferousTrees
        // testcolor
        );
        ...
        const minBend = this.de2ra(corePiece ? -this.options.ANGLE_MIN / 12 : this.options.ANGLE_MIN / 2);
        const maxBend = this.de2ra(corePiece ? -this.options.ANGLE_MAX / 12 : this.options.ANGLE_MAX / 2);
        // TODO This came from the original ConiferousTrees
        ...
        for (let i = 0; i < fullTreeDepth; i++) {
            console.log("-------------branch " + i + ", " + minBend + ",  " + maxBend);
            const trunkBranch = this.buildBranch(branchLength, fullTreeDepth - i, i, fullTreeDepth, minBranchRad * ((fullTreeDepth - i) / fullTreeDepth), maxBranchRad * ((fullTreeDepth - i) / fullTreeDepth));
            ...
            if (i > 0 && depth < 2) {
                for (let j = 0; j < treeData[i].length; j++) {
                    ...
                    const newBranch = this.buildTree(treeData, newBranchL * 0.75, depth + 1, i, fullTreeDepth - 1, (maxBranchRad * ((treeData.length - i) / treeData.length)) / 2, false);
                    ...
                    const branchNode = new three_1.default.Object3D();
     ...
     */
    treeWithRoots(treeData, branchLength, depth, height, fullTreeDepth, maxBranchRad) {
        const body = this.buildTree(treeData, branchLength, 0, height, fullTreeDepth, maxBranchRad, true);
        ...
        for (let i = 0; i < numRoots; i++) {
            ...
            const cylGeom = new three_1.default.CylinderGeometry(rootRad, 0.01, rootLength, 8);
            for (let f = 0; f < cylGeom.faces.length; f++) {
                cylGeom.faces[f].color.setHex(rootColInt);
            }
            const cylMat = new three_1.default.MeshBasicMaterial({
                vertexColors: three_1.default.FaceColors,
                overdraw: 0.5
            });
            const cone = new three_1.default.Mesh(cylGeom, cylMat);
            const newRoot = new three_1.default.Object3D();
    ...
    getTree(options) {
        ...
        //console.trace(data);
        return this.treeWithRoots(data, this.options.BRANCH_L, data.length, 0, data.length, this.options.BRANCH_R_MAX);
    }
    ...
    initOptions(opts) {
        ...
        const bottom_color = this.c.mixHexCols(this.c.brightenByAmt(this.c.randomHex(), -80), "#3e422e", 0.6, 0.4);
...
exports.default = ConiferousTrees;
