...
class Colors {
    ...
    variationsOn(color, variability) {
        ...
        if (!this.isHex(col)) {
            throw new Error(": this version of the code doesn't support web color names anymore.  Please hexify it for now; sorry!");
        }
        else {
            ...
            const newR = Math.max(0, Math.min(255, Math.round(rgbCol.r + rand1 * variability - variability / 2)));
            const newG = Math.max(0, Math.min(255, Math.round(rgbCol.g + rand2 * variability - variability / 2)));
            const newB = Math.max(0, Math.min(255, Math.round(rgbCol.b + rand3 * variability - variability / 2)));
    ...
    randomDarkGrey() {
        ...
        const lowR = Math.max(0, Math.min(255, Math.floor(base + Math.floor(Math.random() * INCR * 2) - INCR)));
        const lowG = Math.max(0, Math.min(255, Math.floor(base + Math.floor(Math.random() * INCR * 2) - INCR)));
        const lowB = Math.max(0, Math.min(255, Math.floor(base + Math.floor(Math.random() * INCR * 2) - INCR)));
        return this.rgbToHex({ r: lowR, g: lowG, b: lowB });
    }
    randomGrey() {
        const INCR = 80;
        const base = Math.floor(Math.random() * INCR * 2);
        const lowR = Math.max(0, Math.min(255, Math.floor(base + Math.floor(Math.random() * INCR) - INCR / 2)));
        const lowG = Math.max(0, Math.min(255, Math.floor(base + Math.floor(Math.random() * INCR) - INCR / 2)));
        const lowB = Math.max(0, Math.min(255, Math.floor(base + Math.floor(Math.random() * INCR) - INCR / 2)));
        return this.rgbToHex({ r: lowR, g: lowG, b: lowB });
    }
    randomDark() {
