function ForestGenerator(forestOptions, treeOptions) {
  ...
  function _initColors() {
    ...
    SKY_COL = NIGHT_MODE
      ? colorHelper.variationsOn("#4d6876", 120)
      : colorHelper.variationsOn("#bdeff1", 150);
    GROUND_COL = NIGHT_MODE
      ? colorHelper.variationsOn("#40523c", 80)
      : colorHelper.variationsOn("#78836e", 150);
    ...
    var vegBase = NIGHT_MODE
      ? GROUND_COL
      : colorHelper.brightenByAmt(GROUND_COL, _r.random(-30, 10));

    var stoneGrey = colorHelper.greyHex(
      NIGHT_MODE ? _r.randomInt(20, 60) : _r.randomInt(90, 130)
    );
  ...
  function _dirtPath() {
    ...
    for (var i = 0; i < 400 / stepSize; i++) {
      ...
      cluster.position.x =
        clusterSpread * _noise[Math.floor(i / wending)] - clusterSpread / 2;
  ...
  function _convertRGBAto8bit(rgbaBuffer, palette) {
    ...
    effectSpacing +=
      spaceSeed < 0.25 ? -1 : spaceSeed < 0.5 ? 0 : spaceSeed < 0.75 ? 2 : 1;
    ...
    var blend0 = colorHelper.parseHex(
      colorHelper.mixHexCols(SKY_COL, GROUND_COL, 0.9, 0.1)
    );
    var blend1 = colorHelper.parseHex(
      colorHelper.mixHexCols(SKY_COL, GROUND_COL, 0.7, 0.3)
    );
    var blend2 = colorHelper.parseHex(
      colorHelper.mixHexCols(SKY_COL, GROUND_COL, 0.5, 0.5)
    );
    var blend3 = colorHelper.parseHex(
      colorHelper.mixHexCols(SKY_COL, GROUND_COL, 0.3, 0.7)
    );
    ...
    for (var i = 0; i < rgbaBuffer.length; i += 4) {
      ...
      if (i > 4 * _sceneWidth && _forestOptions.EFFECT) {
        ...
        var lastPixel = colorHelper.intToHex(
          palette[outputBuffer[(i - 4) / 4]]
        );
        var prevPixel = colorHelper.intToHex(
          palette[outputBuffer[(i - 8) / 4]]
        );
        var abovePixel = colorHelper.intToHex(
          palette[outputBuffer[(i - _sceneWidth * 4) / 4]]
        );
  ...
  function _circleMesh(col, radius, opacity) {
    ...
    var material = new THREE.MeshLambertMaterial({
      color: _rainbow
        ? colorHelper.parseHex(colorHelper.randomHex())
        : colorHelper.parseHex(col),
      transparent: true
    });
  ...
  function _hemisphereMesh(col, radius) {
    var sphGeom = new THREE.SphereGeometry(
      radius,
      5,
      4,
      0,
  ...
  function _bushColors() {
    var cols = [];
    var bushBase = colorHelper.mixHexCols(
      colorHelper.randomHex(),
      _decid.options.COLOR_BTM,
      0.3,
      0.7
    );
  ...
  function _buildRocks() {
    ...
    var baseGrey = colorHelper.greyHex(
      NIGHT_MODE ? _r.randomInt(10, 50) : _r.randomInt(30, 100)
    );
  ...
  function _buildBushes() {
    ...
    var leafSize = _r.random(
      _decid.options.LEAF_SIZE,
      _decid.options.LEAF_SIZE
    );
    ...
    for (var i = 0; i < bushColors.length; i++) {
      backBushCols.push(
        colorHelper.brightenByAmt(bushColors[i], _r.random(-10, 10))
      );
    }
    ...
    for (i = 0; i < numBushes; i++) {
      //console.log("bush "+i);
      var newBush = _bush(
        _r.random(bushHeight * 0.5, bushHeight * 1.2),
        bushWidth,
        bushColors,
        leafSize,
        leafWidth
    ...
    for (i = 0; i < NUM_TREES * 0.6; i++) {
      var newBush = _bush(
        _r.random(bushHeight * 50, bushHeight * 80),
        bushWidth * 8,
        backBushCols,
        leafSize * 4,
        leafWidth
  ...
  function _buildClouds() {
    ...
    for (var i = 0; i < numClumps; i++) {
      ...
      for (var j = 0; j < cloudsPerClump; j++) {
        ...
        cloud.position.x =
          clumpCenterX + _r.random(-cloudMaxRad / 4, cloudMaxRad / 4);
        _forest.add(cloud);
      }
    }
  }
  ...
  function _buildForest() {
    ...
    var planeMat = new THREE.MeshBasicMaterial({
      color: colorHelper.parseHex(GROUND_COL)
    });
    ...
    for (i = 0; i < NUM_TREES * 2; i++) {
      ...
      wrappedTree.scale.x = wrappedTree.scale.y = wrappedTree.scale.z = _r.random(
        0.8,
        1.8
      );
