function ConiferousTrees(options) {
  ...
  function _buildTree(
    ...
    var minBend = _de2ra(
      corePiece ? 180 + _options.ANGLE_MIN / 2 : _options.ANGLE_MIN / 2
    );
    var maxBend = _de2ra(
      corePiece ? 180 + _options.ANGLE_MAX / 2 : _options.ANGLE_MAX / 2
    );
    ...
    for (var i = 0; i < workingDepth; i++) {
      ...
      trunkBranch.rotation.x =
        depth == 0
          ? _r.random(-0.1, 0.1)
          : depth > 1 && i > 1
          ? -_r.random(-minBend / 10, -maxBend / 10)
          : _r.random(-minBend / 10, -maxBend / 10);
      ...
      if (i > 0 && depth < 2) {
        for (var j = 0; j < treeData[i].length; j++) {
          ...
          newBranchL =
            (branchLength * ((treeData.length - i) / treeData.length)) / 3;
          ...
          var newBranch = _buildTree(
            ...
            i,
            fullTreeDepth - 1,
            (maxBranchRad * ((treeData.length - i) / treeData.length)) / 3,
            false
          );
