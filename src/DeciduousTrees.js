function DeciduousTrees(options) {
  ...
  function _pickLeafSize() {
    var sizeRange = Math.random();
    if (sizeRange < 0.33) {
      return _r.random(0.3, 0.5);
    } else if (sizeRange < 0.7) {
      return _r.random(0.5, 0.8);
  ...
  function _randomTreeData(startingStructure, startingDepth) {
    ...
    if (depth < _options.MAX_DEPTH) {
      var branchChance =
        depth == _options.MAX_DEPTH - 1
          ? _options.BRANCH_P
          : _options.BRANCH_P -
            Math.min(_options.BRANCH_P * 0.8, _options.CHANCE_DECAY * depth);

      while (
        _numBranches == 0 ||
        (structure.length < _options.MAX_BRANCHES_PER_NODE &&
          _numBranches < _options.MAX_BRANCHES_TOTAL &&
  ...
  function _makeLeavesAround(
    ...
    for (i = 0; i < numLeaves; i++) {
      ...
      var circleSize = _r.random(1.2, 2.2);
      ...
      newLeaf.position.x += _r.randomSign(
        _r.random(0.75) * circleSize + rAdjust
      );
      newLeaf.position.z += _r.randomSign(
        _r.random(0.75) * circleSize + rAdjust
      );
  ...
  function _buildTree(
    ...
    var fanRads = _de2ra(
      _options.ANGLE_MIN + _r.random(_options.ANGLE_MAX - _options.ANGLE_MIN)
    );
    ...
    for (var i = 0; i < treeData.length; i++) {
      var newBranch = _buildTree(
        treeData[i],
        branchLength * _options.LENGTH_MULT,
        _depthOfArray(treeData[i]),
        height + 1,
        fullTreeDepth,
        maxBranchRad
      );
      ...
      newBranch.rotation.x =
        root.rotation.x + _r.random(-fanRads, fanRads) * fanMod;
      newBranch.rotation.z =
        root.rotation.z + _r.random(-fanRads, fanRads) * fanMod;
  ...
  function _initOptions(opts) {
    ...
    LEAF_BASE_COLOR = nightMode
      ? _c.brightenByAmt(_c.randomHex(), -60)
      : _c.variationsOn(_c.randomHex(), 80);
    var leafColors = [];
    for (i = 0; i < 8; i++) {
      leafColors.push(_c.variationsOn(LEAF_BASE_COLOR, 30));
    }
