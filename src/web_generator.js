...
MAX_BRANCHES_PER_NODE =
  parseInt(getParameterByName("maxbranches")) || MAX_BRANCHES_PER_NODE;
BASE_BRANCH_CHANCE =
  parseFloat(getParameterByName("branchp")) || BASE_BRANCH_CHANCE;
...
function randomTreeData(startingStructure, startingDepth) {
  ...
  if (depth < MAX_DEPTH) {
    var branchChance =
      BASE_BRANCH_CHANCE - Math.min(BASE_BRANCH_CHANCE, CHANCE_DECAY * depth);
    //var branchChance = BASE_BRANCH_CHANCE;

    while (
      structure.length < MAX_BRANCHES_PER_NODE &&
...
function buildBranch(baseLength, distanceFromTip) {
  ...
  var radiusBottom =
    distanceFromTip == treeDepth
      ? 0.75
      : distanceFromTip == treeDepth - 1 && !(distanceFromTip <= 1)
      ? 0.45
      : referenceLength / 17;
  ...
  if (BASE_TREE_COLOR == "silvergreen" || BASE_TREE_COLOR == "silverblack") {
    ...
    BASE_TREE_COLOR == "blackgreen" ||
    BASE_TREE_COLOR == "blacksilver"
  ) {
...
function buildTree(treeData, branchLength, depth) {
  ...
  for (var i = 0; i < treeData.length; i++) {
    var newBranch = buildTree(
      treeData[i],
      branchLength * LENGTH_MULT,
      depthOfArray(treeData[i])
    );
    newBranch.rotation.x =
      root.rotation.x + Math.random() * fanRads - fanRads / 2;
    newBranch.rotation.z =
      root.rotation.z + Math.random() * fanRads - fanRads / 2;
