function ForestGenerator(forestOptions, treeOptions) {
  ...
  function _initColors() {
    ...
    SKY_COL = NIGHT_MODE
      ? _c.variationsOn("#4d6876", 120)
      : _c.variationsOn("#bdeff1", 150);
    GROUND_COL = NIGHT_MODE
      ? _c.variationsOn("#40523c", 80)
      : _c.brightenByAmt(
          _c.variationsOn("#78836e", 150),
          _r.randomInt(-25, -75)
        );
    ...
    var vegBase = NIGHT_MODE
      ? GROUND_COL
      : _c.brightenByAmt(GROUND_COL, _r.random(-30, 10));

    var stoneGrey = _c.greyHex(
  ...
  function _buildForest() {
    ...
    for (i = 0; i < NUM_TREES * 2; i++) {
      ...
      wrappedTree.position.x = _r.randomSign(
        xPositions[i] / 2 + (i < 20 ? _r.random(5, 8) : 0)
      );
      ...
      if (i < NUM_TREES) {
        ...
        wrappedTree.position.x = _r.randomSign(
          xPositions[i] + (i < 12 ? _r.random(3, 5) : 0)
        );
      }
  ...
  function _grassBlade() {
    ...
    for (let i = 0; i < grassHeight; i++) {
      let segment = _grassSegment(
        _treeOptions.BRANCH_L / 8,
        grassHeight - i,
        i,
        grassHeight,
    ...
    for (let i = 0; i < _trees.length; i++) {
      ...
      for (let j = 0; j < patchSize; j += numInCluster) {
        ...
        for (let k = 0; k < numInCluster; k++) {
          ...
          blade.position.x =
            clumpPos.x + _r.randomSign(_r.random(0, clusterSize));
          blade.position.y =
            clumpPos.y + _r.randomSign(_r.random(0, clusterSize));
          blade.position.z =
            clumpPos.z + _r.randomSign(_r.random(0, clusterSize));
